...
void LoginScene::Initialize() {
    ...
    btn->SetOnClickCallback(std::bind(&LoginScene::TextOnClick, this, 1));
    ...
    btn->SetOnClickCallback(std::bind(&LoginScene::TextOnClick, this, 2));
    AddNewControlObject(btn);
    AddNewObject(new Engine::Label("Password", "pirulen.ttf", 48, halfW - 250, halfH * 3 / 2 - 100, 255, 255, 255, 255, 1, 0.5));
    AddNewObject(passwordLabel = new Engine::Label(passwordDisplay, "pirulen.ttf", 48, halfW - 190, halfH * 3 / 2 - 95, 0, 0, 0, 255, 0, 0.5));
    ...
    btn->SetOnClickCallback(std::bind(&LoginScene::LoginOnClick, this, 2));
    AddNewControlObject(btn);
    AddNewObject(new Engine::Label("Login", "pirulen.ttf", 48, halfW + 250, h - 100, 255, 255, 255, 255, 0.5, 0.5));
...
void LoginScene::TextOnClick(int num) {
    if (num == 1) nowEntering = 1;
    else if (num == 2) nowEntering = 2;
}

void LoginScene::BackOnClick(int num) {
    Engine::GameEngine::GetInstance().ChangeScene("start");
}

void LoginScene::LoginOnClick(int num) {
    if (accountData.count(usernameStrokes) == 1) {
        if (accountData[usernameStrokes] == passwordStrokes) {
            StartScene::logged = true;
            StartScene::username = usernameStrokes;
            Engine::GameEngine::GetInstance().ChangeScene("start");
        }
        else {
            usernameStrokes.clear();
            ...
            stateText->Text = "Incorrect Password";
        }
    }
    else {
        usernameStrokes.clear();
        ...
        usernameLabel->Text = usernameStrokes;
        passwordLabel->Text = passwordDisplay;
        stateText->Text = "No Account Found";
    }
}
