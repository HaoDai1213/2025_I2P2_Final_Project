    ...
    for (int i = 0; i < SpeedMult; i++) {
        ...
        switch (current.first) {
            ...
            case 3:
                ...
                break;
            case 4:
                EnemyGroup->AddNewObject(enemy = new MechEnemy(SpawnCoordinate.x, SpawnCoordinate.y, 1));
                break;
            default:
                continue;
        }
...
void PlayScene::OnMouseUp(int button, int mx, int my) {
    ...
    if (button & 1) {
        if (shovelToggle) {
            Engine::LOG(Engine::INFO) << "Shovel Toggled";
            if (mapState[y][x] == TILE_OCCUPIED) {
                if (!preview)
                    return;
                for (auto &it : TowerGroup->GetObjects()) {
                    int cx = floor(it->Position.x / BlockSize);
                    int cy = floor(it->Position.y / BlockSize);
                    if (cx == x && cy == y) {
                        Engine::LOG(Engine::INFO) << "founded";
                        tbdTurret = dynamic_cast<Turret*>(it);  // base(IbOject) -> derived(Turret)
                        break;
                    }
                }
                if (!tbdTurret) {
                    Engine::LOG(Engine::ERROR) << "dynamic_cast failed";
                    return;
                }
                // Purchase.
                EarnMoney(tbdTurret->GetPrice() / 2);
                // Remove Preview.
                preview->GetObjectIterator()->first = false;
                UIGroup->RemoveObject(preview->GetObjectIterator());
                // Remove Turret.
                TowerGroup->RemoveObject(tbdTurret->GetObjectIterator());
                // To keep responding when paused.
                preview->Update(0);
                // Remove Preview.
                preview = nullptr;
...
void PlayScene::OnKeyDown(int keyCode) {
    ...
    if (keyCode == ALLEGRO_KEY_TAB) {
        ...
        if (keyStrokes.size() == code.size()) {
            ...
            if (cheat) {
                Engine::LOG(Engine::INFO) << "Cheat Mode On";
                EarnMoney(10000);
...
void PlayScene::ConstructUI() {
    ...
    btn = new TurretButton("play/floor.png", "play/dirt.png",
                           ...
                           Engine::Sprite("play/turret-6.png", 1446, 136 - 8, 0, 0, 0, 0), 1446, 136, MissileTurret::Price);
    btn->SetOnClickCallback(std::bind(&PlayScene::UIBtnClicked, this, 2));
    UIGroup->AddNewControlObject(btn);
    // Button 4
    btn = new TurretButton("play/floor.png", "play/dirt.png",
                           Engine::Sprite("play/null.png", 1446, 364, 0, 0, 0, 0),
                           Engine::Sprite("play/shovel.png", 1446, 364 - 8, 0, 0, 0, 0), 1446, 364, Shovel::Price);
...
std::vector<std::vector<int>> PlayScene::CalculateBFSDistance() {
    ...
    while (!que.empty()) {
        ...
        for (auto& it : directions) {
            Engine::Point np = p + it;
            if (np.x >= 0 && np.x <= MapWidth - 1 && np.y >= 0 && np.y <= MapHeight - 1) {
                if (map[np.y][np.x] == -1 && mapState[np.y][np.x] == TILE_DIRT) {
                    map[np.y][np.x] = map[p.y][p.x] + 1;
                    que.push(Engine::Point(np.x, np.y));
                }
            }
        }
    }
